package com.billy.android.soso.framwork.ui.adapter;
 ...
 * Author：summer
 * Description：
 * 1、对ListAdapter 进行简易封装，对ListAdapter 本身是默认使用最大2个线程的 ，只需要实现DiffUtil.ItemCallback 差异化比对接口
 * 2、提供差异化分析功能， 并提供线程池子线程分析， handler 通知更新ui
 * 3、DataBindingUtil view，  注册点击事件，BaseBindingViewHolder填充数据
 */
public abstract class BaseDataBindingAdapter<M, B extends ViewDataBinding> extends ListAdapter<M, RecyclerView.ViewHolder> {
    ...
    /**
     * 调用 submitList() 方法传递数据集时，需要new一个新的List
